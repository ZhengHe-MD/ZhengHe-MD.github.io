...
Personal digital garden and publication system for essays, interactive courses, projects, running logs, talks, and micro-notes.
...
- **Content folders are immutable sources of truth.** `scripts/build.mjs` is the sole build step. It scans content folders (`writing/`, `courses/`, `til/`), parses `<meta>` tags, and generates aggregates into `_site/` (`index.html`, `/writing/`, `/courses/`, `/til/`, `/categories/`, `/feed.xml`, and forwarder stubs). It never writes back to source folders.
...
- **TIL (`til/<slug>/index.html`)**: Micro-notes. Body snippet is delimited by `<!-- til:body -->` ... `<!-- /til:body -->` for automatic stream inlining at `/til/`.
